...
1) **[666 is the result of an arithmetic calculation](calculate_the_number_of_the_wild_beast.py)**, not the base for [gematria](doc/Create gematria_wrong_approach.md) or a numerological number puzzle. The Greek word used for "calculate" in Revelation 13:18 is ψηφισάτω (psephisatō), which comes from the root ψηφίζω (psephizō). This word has a double meaning for mathematical calculation: counting/computing with pebbles (like an abacus) and also decision-making through voting by casting pebbles into urns to make choices.
